...
- 🔻 **Seller ATP DKPC:** نسخه‌ی **سطح DKPC** تب Per-Seller Item-Tail — همون قانون شناسایی و همون بج ST/MT/LT به‌ازای هر فروشنده، ولی یک ردیف به ازای هر **واریانت (DKPC)** و با موجودی **وزن‌آگاه** سطح DKPC. هر ردیف وضعیت **Available / Unavailable** خودش رو هم نشون میده، پس هم کالاهای همچنان فعال و هم غیرفعال‌ها تو یک جدول دیده میشن (به ازای هر فروشنده، اول غیرفعال‌ها و بعد ST → MT → LT). رنگی بر اساس بج و وضعیت.
  - 📦 **خروجی ZIP جدا برای هر فروشنده** (اختیاری): همون ردیف‌ها، فقط **غیرفعال‌ها** (لیست اقدام‌پذیر «اینا رو دوباره لایو کن») — یک اکسل به ازای هر فروشنده با نام `SellerID-SellerName.xlsx` با وزن، دسته‌بندی، بج شمش/زیور و بج دم‌دراز — آماده برای ایمیل مستقیم به فروشنده.
...
**قاعده‌ی مرز:** معیار دسته‌بندی هر کالا، سهمی از حجم فروشه که **بالای** اون کالا جمع شده — نه سهمی که با احتساب خودش به دست میاد. یعنی کالایی که روی مرز می‌افته، جزو دسته‌ای حساب میشه که ازش **شروع** شده، نه دسته‌ای که بهش سرریز می‌کنه. نتیجه‌ش اینه که پرفروش‌ترین کالای هر بخش همیشه ST هست، حتی اگه خودش به‌تنهایی بیشتر از ۳۰٪ حجم فروش رو بگیره. (بدون این قاعده، تو تب چهارم که رتبه‌بندی به ازای هر فروشنده و فقط روی چند ده کالاست، کالای اولِ فروشنده تقریباً همیشه از ۳۰٪ رد می‌شد و می‌افتاد تو MT یا LT — و اون فروشنده اصلاً هیچ ST نمی‌گرفت.)
...
### ۴ج. تب Seller ATP DKPC — همون منطق، ولی در سطح DKPC

این تب دقیقاً همون **منطق شناسایی کالا و رتبه‌بندی** تب Per-Seller Item-Tail رو داره (بج ST/MT/LT از رتبه‌بندی مستقل خودِ فروشنده، و DKPهای بدون بج — پیش‌بینی فروش صفر/خالی — اصلاً وارد گزارش نمیشن)، با دو تفاوت:

1. **لایه‌ی بررسی، DKPC هست نه DKP:** هر ردیف یک واریانت کالاست و موجودی/ATP اون با منطق **وزن‌آگاه سطح DKPC** حساب میشه (تطبیق دقیق DKPC، یا تقریب وزنی همون DKP) — نه منطق مستقل از وزنِ سطح DKP.
2. **وضعیت فعال/غیرفعال هر ردیف نمایش داده میشه:** ستون `Status` مقدار `Available` یا `Unavailable` می‌گیره، پس کالاهایی که هنوز لایو هستن و کالاهایی که از دست رفتن، کنار هم تو یک جدول دیده میشن.

مرتب‌سازی: به ازای هر فروشنده، اول ردیف‌های `Unavailable` و بعد `Available`؛ و داخل هر گروه به ترتیب ST → MT → LT، بعد DKPC. ستون‌های خروجی: `Seller ID`, `Seller`, `DKP`, `DKP Name`, `DKPC`, `Category`, `Bucket`, `Tail Badge`, `Status`.

چون یک DKP می‌تونه یک واریانت لایو و یک واریانت از دست رفته داشته باشه، طبیعیه که همون DKP تو تب Per-Seller Item-Tail (سطح DKP) `Available` باشه ولی این‌جا بعضی واریانت‌هاش `Unavailable` باشن — این دقیقاً همون چیزیه که سطح DKPC نشون میده.

### ۵. خروجی جدا برای هر فروشنده (فایل ZIP)

اگه گزینه‌ی «تولید فایل ZIP جدا برای هر فروشنده» رو فعال کنید، یک فایل zip می‌سازه که توش به ازای هر فروشنده یک اکسل با نام `SellerID-SellerName.xlsx` هست — شامل کالاهای غیرفعال (NOT ATP) همون فروشنده، با اسم کالا (DKP Name)، وزن، دسته‌بندی، بج شمش/زیور و بج دم‌دراز، مرتب‌شده بر اساس مقدار پیش‌بینی فروش (که خودش تو خروجی نمیاد). این خروجی همون ردیف‌های جدول Seller ATP DKPC هست، فقط محدود به غیرفعال‌ها — یعنی DKPهای بدون بج (پیش‌بینی فروش صفر/خالی) این‌جا هم نمیان. این فایل برای ایمیل کردن مستقیم به هر فروشنده طراحی شده و فقط در صورت درخواست ساخته میشه (پردازش اضافه‌ست).
...
│   ├── missing_generator.py    ساخت جدول Seller ATP DKPC (سطح DKPC، با وضعیت)
│   ├── report_labels.py        واژگان مشترک Available/Unavailable و رنگ بج/وضعیت
...
| `DKP Name` | — | **اسم کالا** — اختیاری؛ فقط برای نمایش تو خروجی‌ها |
| `Weight` | `product_variant_name_fa` | وزن (هر دو با یک منطق پارس میشن) |
...
**تطبیق نام ستون‌ها سخت‌گیر نیست:** تفاوت حروف بزرگ/کوچک، فاصله یا `_`/`-`، و فاصله‌ی اضافه‌ی اول/آخر نادیده گرفته میشه. یعنی `DKP Name`، `DKP_Name`، `DKP NAME`، `dkp name` و `"DKP Name "` همگی یک ستون حساب میشن. برای ستون اسم کالا چند املای رایج دیگه (مثل `Product Name` یا `نام کالا`) هم به‌صورت خودکار تست میشه. اگه هیچ‌کدوم پیدا نشد، تو هشدارهای همون اجرا **لیست ستون‌های واقعی فایل** نشون داده میشه تا بتونید درستش رو تو تنظیمات بذارید.

**اسم کالا (`DKP Name`)** از فایل اسورتمنت (Live_Data) خونده میشه و تو همه‌ی خروجی‌هایی که DKP دارن، کنار DKP نمایش داده میشه. این ستون **اختیاری**ه: اگه تو فایل نبود، فایل رد نمیشه — فقط یک هشدار می‌گیرید و ستون اسم کالا خالی می‌مونه. نگاشت اسم بر اساس **خود DKP** (مستقل از فروشنده) انجام میشه، چون DKP شناسه‌ی سطح مارکت‌پلیسه: همین باعث میشه اسم کالاهای غیرفعالِ یک فروشنده هم — که دقیقاً موضوع گزارش Missing هستن — از ردیف لایو فروشنده‌های دیگه پیدا بشه. DKPیی که هیچ فروشنده‌ای لایو نداره، اسمش خالی می‌مونه.

اگه فروشنده اسم یکی از این ستون‌ها رو تغییر بده (مثلاً همین ستون وزن که قبلاً `Size_Name` بود و الان `Weight` شده)، لازم نیست کد رو دستکاری کنید: از بخش «⚙️ تنظیمات — نام ستون‌های اکسل» در بالای اپ می‌تونید اسم هر ستون رو اصلاح کنید. این مقادیر روی همون سیستم ذخیره می‌شن (تو `~/.atp_analyzer/field_names.json`) و با هر بار اجرای اپ دوباره بارگذاری می‌شن — دیگه لازم نیست هر بار وارد کنید.
